/* Возрождение — рассылка инвестиционным отделам банков: рендер карточек,
   генерация писем/ссылок, чекбоксы "отправлено"/"прозвонили" с датой в
   localStorage, напоминание на следующий рабочий день. Точная копия
   assets/js/buyers.js (префикс bk- вместо by-). */
(function () {
  'use strict';

  var KEY = 'vozrozhdenie_banks_mailer_v1';
  ...
  function backupFileName() {
    var d = new Date();
    return 'vozrozhdenie-banks-backup-' + d.getFullYear() + pad2(d.getMonth() + 1) + pad2(d.getDate())
  ...
  function showBackupStatus(text, isError) {
    var el = document.getElementById('bkBackupStatus');
    if (!el) return;
    el.textContent = text;
    el.hidden = false;
  ...
  function emailBackup() {
    ...
    var subject = 'Возрождение — резервная копия отметок банков';
    var body;
    if (compactJson.length > 1800) {
      ...
      body = 'Резервная копия отметок по банкам (JSON). Чтобы восстановить на другом устройстве — '
  ...
  function importBackupFile(file) {
    ...
    reader.onload = function () {
      ...
      allBanks.forEach(function (b) { frag.appendChild(renderCard(b, merged)); });
  ...
  function buildMailto(b) {
    var subject = 'Возрождение — инвестиционный актив у реки Воронеж, Липецкая область';
  ...
  function renderCard(b, state) {
    ...
    var contactsHtml = '';
    if (b.email) {
      contactsHtml += '<a class="bk-chip mail js-channel-chip' + clickedClass('mail') + '" data-channel="mail" href="' + buildMailto(b) + '">'
        + clickedMark('mail') + '&#9993; Написать письмо</a>';
    } else {
      contactsHtml += '<span class="bk-chip missing">email не найден</span>';
    }
    ...
    if (b.phone) {
      contactsHtml += '<a class="bk-chip js-channel-chip' + clickedClass('tel') + '" data-channel="tel" href="tel:+' + digits + '">'
        + clickedMark('tel') + '&#9742; ' + b.phone + '</a>';
      if (isMobileLike) {
        contactsHtml += '<a class="bk-chip tg js-msg-chip js-channel-chip' + clickedClass('tg') + '" data-channel="tg"'
        ...
        contactsHtml += '<a class="bk-chip wa js-msg-chip js-channel-chip' + clickedClass('wa') + '" data-channel="wa"'
        ...
        contactsHtml += '<button type="button" class="bk-chip max js-max-chip js-channel-chip' + clickedClass('max') + '" data-channel="max"'
      ...
      contactsHtml += '<span class="bk-chip missing">телефон не найден</span>';
    }
    if (b.website) {
      var host = b.website.replace(/^https?:\/\//, '').replace(/\/$/, '');
      contactsHtml += '<a class="bk-chip" href="' + b.website + '" target="_blank" rel="noopener">' + host + '</a>';
    }

    var contactPerson = b.contactName
      ? '<div class="bk-meta">' + b.contactName + (b.contactPosition ? ' — ' + b.contactPosition : '') + '</div>'
      : '';
    ...
    if (baseAt && !st.calledAt) {
      ...
      if (Date.now() >= due) {
        isDue = true;
        dueBadge = '<span class="bk-duebadge">&#128222; Пора позвонить</span>';
      }
    }
    var bounceBadge = st.bounced ? '<span class="bk-bouncebadge">&#9888; Письмо не дошло</span>' : '';
    ...
    var sentInfo = infoBits.length ? '<span class="bk-sentinfo">' + infoBits.join(' &middot; ') + '</span>' : '';
    ...
    if (!hasContactHistory) {
      ...
      meetingRow = '<div class="bk-meeting-row">' +
        '<span class="bk-meeting-badge">&#128197; Встреча: ' + st.meeting.date + ' ' + st.meeting.time + ' &middot; ' + st.meeting.place +
        ' <button type="button" class="bk-meeting-edit js-meeting-edit">изменить</button></span>' +
        '<div class="bk-meeting-form" id="mf-' + b.rank + '" hidden>' +
      ...
      meetingRow = '<div class="bk-meeting-row">' +
        '<button type="button" class="bk-meeting-btn js-meeting-open">&#128197; Назначить встречу</button>' +
        '<div class="bk-meeting-form" id="mf-' + b.rank + '" hidden>' +
    ...
    card.innerHTML =
      '<div class="bk-top">' +
        '<div>' +
          '<div class="bk-name"><span class="bk-rank">#' + b.rank + '</span> ' + b.company + '</div>' +
          '<div class="bk-meta">' + b.region + ' &middot; ' + b.profile + '</div>' +
          contactPerson +
        '</div>' +
        '<span class="bk-badge ' + priorityClass(b.priority) + '">' + b.priority + '</span>' +
      '</div>' +
      ...
      '<div class="bk-card-body"' + (st.refused ? ' hidden' : '') + '>' +
        ...
        '<div class="bk-bottom">' +
          '<div class="bk-checks">' +
            '<label class="bk-check"><input type="checkbox" class="js-sent" ' + (st.sentAt ? 'checked' : '') + '> Письмо отправлено</label>' +
            (st.sentAt ? '<label class="bk-check bk-check-warn"><input type="checkbox" class="js-bounced" ' + (st.bounced ? 'checked' : '') + '> Письмо не дошло (ящика не существует)</label>' : '') +
            '<label class="bk-check"><input type="checkbox" class="js-corr" ' + (st.corrAt ? 'checked' : '') + '> Переписка с менеджером</label>' +
            (showAppSelect ? (
              '<select class="bk-app-select js-app">' +
            ...
            (showCalled ? '<label class="bk-check"><input type="checkbox" class="js-called" ' + (st.calledAt ? 'checked' : '') + '> Прозвонили</label>' : '') +
          '</div>' +
    ...
    var meetingForm = card.querySelector('.bk-meeting-form');
  ...
  function rerenderCard(b, state) {
    var old = document.querySelector('.bk-card[data-rank="' + b.rank + '"]');
    if (!old) return;
  ...
  function updateStats() {
    ...
    ['bkTotal', 'bkTotal2', 'bkTotal3'].forEach(function (id) {
      var el = document.getElementById(id);
      if (el) el.textContent = total;
    });
    var elSent = document.getElementById('bkSent');
    var elDue = document.getElementById('bkDue');
    var elLeft = document.getElementById('bkLeft');
  ...
  var FILTER_KEY = 'vozrozhdenie_banks_filters_v1';
  ...
  function currentFilterValues() {
    return {
      q: document.getElementById('bkSearch').value || '',
      prio: document.getElementById('bkPrioFilter').value || '',
      status: document.getElementById('bkStatusFilter').value || '',
      email: document.getElementById('bkEmailFilter').value || ''
    };
  }

  function updateFilterBadge(vals) {
    var count = (vals.q ? 1 : 0) + (vals.prio ? 1 : 0) + (vals.status ? 1 : 0) + (vals.email ? 1 : 0);
    var badge = document.getElementById('bkFilterBadge');
    var clearBtn = document.getElementById('bkClearFiltersBtn');
  ...
  function applyFilters() {
    ...
    var empty = document.getElementById('bkEmpty');
    if (empty) empty.hidden = visible !== 0;
    var countEl = document.getElementById('bkVisibleCount');
  ...
  function setPanelOpen(open) {
    var panel = document.getElementById('bkFilterPanel');
    var toggle = document.getElementById('bkToggleBtn');
    panel.hidden = !open;
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    var list = document.getElementById('bkList');
    var frag = document.createDocumentFragment();
    allBanks.forEach(function (b) { frag.appendChild(renderCard(b, state)); });
    list.appendChild(frag);
    updateStats();

    // Восстановить сохранённые фильтры и состояние панели
    var prefs = loadFilterPrefs();
    if (prefs.q) document.getElementById('bkSearch').value = prefs.q;
    if (prefs.prio) document.getElementById('bkPrioFilter').value = prefs.prio;
    if (prefs.status) document.getElementById('bkStatusFilter').value = prefs.status;
    if (prefs.email) document.getElementById('bkEmailFilter').value = prefs.email;
    ...
    document.getElementById('bkPrioFilter').addEventListener('change', applyFilters);
    document.getElementById('bkStatusFilter').addEventListener('change', applyFilters);
    document.getElementById('bkEmailFilter').addEventListener('change', applyFilters);

    document.getElementById('bkToggleBtn').addEventListener('click', function () {
      var panel = document.getElementById('bkFilterPanel');
      setPanelOpen(panel.hidden);
    });

    document.getElementById('bkClearFiltersBtn').addEventListener('click', function () {
      document.getElementById('bkSearch').value = '';
      document.getElementById('bkPrioFilter').value = '';
      document.getElementById('bkStatusFilter').value = '';
      document.getElementById('bkEmailFilter').value = '';
      applyFilters();
    });

    var exportBtn = document.getElementById('bkExportBtn');
    if (exportBtn) exportBtn.addEventListener('click', exportBackup);

    var emailBtn = document.getElementById('bkEmailBtn');
    if (emailBtn) emailBtn.addEventListener('click', emailBackup);

    var importInput = document.getElementById('bkImportInput');
    if (importInput) {
    ...
    document.getElementById('bkResetBtn').addEventListener('click', function () {
      if (!confirm('Сбросить все отметки "отправлено"/"прозвонили" по всем ' + allBanks.length + ' записям? (Фильтры это не затронет.)')) return;
      saveState({});
      list.innerHTML = '';
      var frag2 = document.createDocumentFragment();
      allBanks.forEach(function (b) { frag2.appendChild(renderCard(b, {})); });
